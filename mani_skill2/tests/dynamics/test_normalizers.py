...
class TestAffineNormalizer:
    ...
    def test_normalize_denormalize(self, device):
        ...
        dut = AffineNormalizer(lo, up)

        np.testing.assert_allclose(
            dut.normalize(lo).cpu().detach().numpy(), np.array([-1.0, -1.0])
        )
        np.testing.assert_allclose(
            dut.normalize(up).cpu().detach().numpy(), np.array([1.0, 1.0])
        )
        np.testing.assert_allclose(
            dut.denormalize(torch.tensor([-1., -1], device=device)).cpu().detach().numpy(),
            lo.cpu().detach().numpy(),
        )
        np.testing.assert_allclose(
            dut.denormalize(torch.tensor([1., 1], device=device)).cpu().detach().numpy(),
        ...
        x = torch.tensor([[-3, 5.], [1, 2], [-3, 7]], device=device)
    ...
    def test_unscaled(self, device):
        ...
        """
        lo = torch.tensor([0, 2, 3.])
        up = torch.tensor([0, 5., 3.])
        dut = AffineNormalizer(lo, up)

        x = torch.tensor([[2, 5, 1], [0, 2, 2.]], device=device)
        x_normalized = dut.normalize(x)
        np.testing.assert_allclose(x_normalized[:, 0].cpu().detach().numpy(), x[:, 0].cpu().detach().numpy())
        np.testing.assert_allclose(x_normalized[:, 2].cpu().detach().numpy(), x[:, 2].cpu().detach().numpy())
        np.testing.assert_allclose(x_normalized[:, 1].cpu().detach().numpy(), np.array([1, -1.]))
        x_denormalized = dut.denormalize(x_normalized)
        np.testing.assert_allclose(x_denormalized.cpu().detach().numpy(), x.cpu().detach().numpy())

    def test_from_data(self):
        data = torch.tensor([[-3., 2.], [-1, 5], [0, 4]])
        dut = AffineNormalizer.from_data(data)
        np.testing.assert_allclose(dut.lo.cpu().detach().numpy(), np.array([-3, 2.]))
        np.testing.assert_allclose(dut.up.cpu().detach().numpy(), np.array([0., 5.]))
